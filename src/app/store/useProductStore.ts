...
import { ref } from "vue";
import { TItem, TProduct, TTag } from "../types/TProduct";

export const useProductStore = defineStore("useTags", () => {
  const products = ref<TProduct[]>([]);
  const selectedTag = ref<TTag[]>([]);
  ...
  const getData = (): Promise<TProduct[]> => {
  ...
  const getTags = (line: string): TTag[] => {
    return findTagsByString(products.value, line);
  };

  function findTagsByString(data: TItem[], searchString: string): TTag[] {
    let foundTags: TTag[] = [];

    for (const item of data) {
      if (item.categories) {
        foundTags = foundTags.concat(
          findTagsByString(item.categories, searchString)
        );
      }
      if (item.subcategories) {
        foundTags = foundTags.concat(
          findTagsByString(item.subcategories, searchString)
        );
      }
      if (item.tags) {
        foundTags = foundTags.concat(findTagsByString(item.tags, searchString));
      }
      if (item.aliases) {
        foundTags = foundTags.concat(item as TTag);
        foundTags = foundTags.filter((el) => {
          if (el.name.toUpperCase().includes(searchString.toUpperCase())) {
            return true;
          } else if (
            el.aliases.find((el: string) =>
              el.toUpperCase().includes(searchString.toUpperCase())
            )
          ) {
            return true;
          } else {
  ...
  const addTag = (tags: TTag[]) => {
    console.log(tags);
    selectedTag.value = [...selectedTag.value, ...tags];
  };
